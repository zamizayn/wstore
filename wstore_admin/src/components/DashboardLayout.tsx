...
import { LayoutDashboard, Tags, ShoppingBag, ShoppingCart, Users, LogOut, Hexagon, MapPin, Building2, ChevronDown, Boxes, LifeBuoy, Search, Bell, Settings, CreditCard, TrendingUp, ArrowRight, X, Menu, MessageSquare, Lock, RefreshCw, Bike } from 'lucide-react';
...
const ALL_PAGES = [
    ...
    { label: 'Subscription Plans', path: '/admin/subscription-plans', icon: CreditCard, keywords: ['subscription', 'plan', 'pricing', 'billing', 'tier'], roles: ['superadmin'] },
    { label: 'Subscriptions', path: '/admin/subscriptions', icon: RefreshCw, keywords: ['subscription', 'renew', 'cancel', 'upgrade', 'downgrade', 'billing'], roles: ['superadmin', 'tenant', 'branch'] },
    ...
    { label: 'Delivery Boys', path: '/admin/delivery-boys', icon: Bike, keywords: ['delivery', 'rider', 'boy', 'dispatch', 'driver'], roles: ['superadmin', 'tenant', 'branch'] },
...
export default function DashboardLayout() {
    ...
    // Fetch tenant details on mount to sync logo
    useEffect(() => {
        if (role === 'tenant' || role === 'branch') {
            fetch(`${API_ENDPOINTS.TENANTS}/me`, { headers: getHeaders() })
                .then(res => res.json())
                .then(data => {
                    const logoUrl = data.whatsappSettings?.logo || '';
                ...
                .catch(err => console.error('Error fetching tenant logo:', err));
        }
    }, [role]);

    // Listen for custom logo update events from settings
    useEffect(() => {
        const handleLogoUpdate = (e) => {
            if (e.detail?.logo !== undefined) {
                setTenantLogo(e.detail.logo);
            }
        };
        ...
        <div className="app-container">
            <aside className={`sidebar ${!isSidebarOpen ? 'hidden' : ''}`}>
                <div className="sidebar-header">
                    <div className="brand-logo" style={{ overflow: 'hidden', borderRadius: '8px' }}>
                        <img src={tenantLogo || logo} alt="Logo" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                ...
                <div className="sidebar-nav">
                    ...
                    {role === 'superadmin' && (
                        <>
                            ...
                            <NavLink to="/admin/subscription-plans" className={({ isActive }) => `sidebar-nav-link ${isActive ? 'active' : ''}`}>
                                <CreditCard size={18} /> <span>Subscription Plans</span>
                            </NavLink>
                            <NavLink to="/admin/subscriptions" className={({ isActive }) => `sidebar-nav-link ${isActive ? 'active' : ''}`}>
                                <RefreshCw size={18} /> <span>Subscriptions</span>
                            </NavLink>
                    ...
                    {(role === 'tenant' || role === 'branch') && (
                        <NavLink to="/admin/subscriptions" className={({ isActive }) => `sidebar-nav-link ${isActive ? 'active' : ''}`}>
                            <RefreshCw size={18} /> <span>Subscription</span>
                        </NavLink>
                    )}
